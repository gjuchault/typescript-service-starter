...
5. `npm run setup`
6. `echo "COOKIE_SIGNING_SECRET=$(openssl rand -hex 32)" > .env.local`
...
This template is based on Fastify with some nice defaults (circuit breaker, rate limit, etc.). [ts-rest](https://ts-rest.com/) is used to have nice routes & automatic client generations with zod and TypeScript.
It leverages PostgreSQL as a storage (through [slonik](https://github.com/gajus/slonik)).
